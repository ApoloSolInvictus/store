...
jQuery( document ).ready(
	function() {
		...
		stargaze_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="stargaze_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.stargaze_post_editor' );
						if ( link.length > 0 ) {
							stargaze_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.stargaze_post_editor', function(e) {
						stargaze_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('stargaze_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function stargaze_change_post_edit_link_elementor(a) {
			if (a.length > 0) {

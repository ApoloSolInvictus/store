...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			stargaze_create_observer( 'stargaze-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				stargaze_gutenberg_editor_init();
			} );
		} else {
		 	stargaze_gutenberg_editor_init();
		}
	...
	function stargaze_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'stargaze_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'stargaze_inited' ) ) {
	...
	// Init on page load
	function stargaze_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = stargaze_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + STARGAZE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + STARGAZE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in STARGAZE_STORAGE['override_classes'] ) {
				$obj.addClass( STARGAZE_STORAGE['override_classes'][i].replace( '%s', STARGAZE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( stargaze_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && STARGAZE_STORAGE['sidebar_position'] != 'hide' ) {
				if ( stargaze_apply_filters( 'stargaze_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'stargaze_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( stargaze_apply_filters( 'stargaze_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				stargaze_remove_observer( 'stargaze-check-editor-styles-wrapper' );
				stargaze_create_observer( 'stargaze-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'stargaze_inited' );
	}
} );

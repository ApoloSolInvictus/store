/* global stargaze_color_schemes, stargaze_dependencies, Color */
...
( function( api ) {
	...
	for (var i in stargaze_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'stargaze-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'stargaze-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'stargaze-vars' );

	// Allow an alpha channel in the color picker
	if ( stargaze_customizer_vars['colorpicker_allow_alpha'] ) {
	...
	api.bind( 
		'ready', function() {
			...
			jQuery( '#customize-header-actions #save' )
				.before( '<input type="button" class="button customize-action-reset" value="' + stargaze_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' )
				.on( 'click', function(e) {
					if (confirm( stargaze_customizer_vars['msg_reset_confirm'] )) {
			...
			jQuery( '#customize-header-actions .spinner' )
				.after( '<button class="button customize-action-refresh icon-spin3" title="' + stargaze_customizer_vars['msg_reload_title'] + '">' + stargaze_customizer_vars['msg_reload'] + '</button>' );
			...
			if (stargaze_customizer_vars && stargaze_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( stargaze_customizer_vars['theme_name_suffix'] );
			}
			...
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' )
				.on( 'change', stargaze_customizer_update_load_fonts );
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' )
				.on( 'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							stargaze_customizer_check_dependencies( section );
						}
					}
				} );

			// Decorate subsections in the panel 'Typography'
			if ( stargaze_customizer_vars['decorate_fonts'] ) {

				jQuery('body').addClass( 'stargaze_decorate_fonts_section' );

				// Prepare section for open on title is clicked
				var stargaze_customizer_title_clicked = function( title ) {
					var id = title.parent().attr( 'aria-owns' );
					stargaze_customizer_close_opened_sections( title.parents( '#sub-accordion-panel-fonts' ), id );
					var section = jQuery( '#' + id ),
					...
					.each ( function() {
						...
						$self
							.on( 'keydown', function(e) {
								if ( e.which == 13 ) {
									stargaze_customizer_title_clicked( $self );
								}
							} )
							.on( 'click', function() {
								stargaze_customizer_title_clicked( $self );
				...
				var stargaze_customizer_close_opened_sections = function( panel, current_id ) {
					if ( panel.length ) {
				...
				jQuery( '#customize-theme-controls #sub-accordion-panel-fonts .customize-panel-back' )
					.on( 'keydown', function(e) {
						if ( e.which == 13 ) {
							stargaze_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
						}
					} )
					.on( 'click', function() {
						stargaze_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
	...
	api.bind( 'change', function(obj) {
		...
		stargaze_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
		stargaze_customizer_refresh_preview( obj );
	} );
	...
	api.bind( 'refresh_schemes', function() {
		...
		for (i in cssTemplate) {
			...
			i = i.replace( 'scheme_', '' );
			if (typeof stargaze_color_schemes[i] === 'undefined') {
				delete cssTemplate['scheme_' + i];
				jQuery( '#tmpl-stargaze-color-scheme-' + i ).remove();
			} else if (tpl_idx == '') {
				tpl_idx     = i;
				tpl_content = jQuery( '#tmpl-stargaze-color-scheme-' + i ).html().trim();
			}
		}
		// Add new templates
		var regex = new RegExp( "\.scheme_" + tpl_idx, "g" );
		for (i in stargaze_color_schemes) {
			if (typeof cssTemplate['scheme_' + i] === 'undefined') {
				jQuery( '#tmpl-stargaze-color-scheme-' + tpl_idx )
				.clone()
				.attr( 'id', 'tmpl-stargaze-color-scheme-' + i )
				.html( tpl_content.replace( regex, '.scheme_' + i ) )
				.insertAfter( jQuery( '#tmpl-stargaze-color-scheme-' + tpl_idx ) );
				cssTemplate['scheme_' + i] = wp.template( 'stargaze-color-scheme-' + i );
			}
		}
	} );
	...
	for (var action in stargaze_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (stargaze_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = stargaze_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = stargaze_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function stargaze_customizer_get_field_value(fld) {
	...
	function stargaze_customizer_check_dependencies(cont) {
		if ( typeof stargaze_dependencies == 'undefined' || STARGAZE_STORAGE['check_dependencies_now'] ) return;
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = stargaze_customizer_get_field_value( fld );
							if ( val == 'inherit' ) {
								dep_cnt = 0;
								dep_all = 1;
								var tag;
	...
	function stargaze_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( stargaze_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					stargaze_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in stargaze_theme_fonts) {
					for (rule in stargaze_theme_fonts[opt]) {
						if (opt + '_' + rule.replace( ':', '-' ) == id) {
							// Store new value to the fonts table
							stargaze_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
			...
			if (processed || forceUpdateCSS) {
				stargaze_customizer_update_css();
			}
		}
	...
	// Actions buttons
	function stargaze_customizer_actions(e) {
	...
	function stargaze_customizer_update_theme_vars(opt, value) {
		stargaze_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function stargaze_customizer_update_theme_fonts(opt, rule, value) {
		stargaze_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function stargaze_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= stargaze_customizer_vars['max_load_fonts']; i++) {
	...
	function stargaze_customizer_update_css() {
		...
		var vars = stargaze_customizer_add_theme_vars( stargaze_theme_vars );

		// Make styles and add its to the css
		css += stargaze_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = stargaze_customizer_add_theme_fonts( stargaze_theme_fonts );

		// Make styles and add its to the css
		css += stargaze_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in stargaze_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( stargaze_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in stargaze_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in stargaze_color_schemes[scheme].colors) {
				colors[i] = stargaze_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = stargaze_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-stargaze-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function stargaze_customizer_add_theme_colors(colors) {
		if (stargaze_additional_colors) {
			var clr = '', v = '';
			for (var k in stargaze_additional_colors) {
				v   = stargaze_additional_colors[k];
				clr = colors[v['color']];
				if ( clr.slice( 0, 3 ) == 'rgb' ) {
					clr = stargaze_rgba2hex( clr );
				}
				if (typeof v['hue'] != 'undefined' || typeof v['saturation'] != 'undefined' || typeof v['brightness'] != 'undefined') {
					clr = stargaze_hsb2hex(
						stargaze_hex2hsb(
							clr,
	...
	function stargaze_customizer_add_theme_fonts(fonts) {
		var rez = [];
			...
			for (var css_prop in fonts[tag]) {
				...
				rez[tag + '_' + css_name] = css_value !== '' && css_value != 'inherit'
												? css_prop + ':' + ( ['font-size', 'letter-spacing', 'margin-top', 'margin-bottom', 'border-width', 'border-radius'].indexOf(css_prop) >= 0
																		? stargaze_customizer_prepare_css_value( css_value )
																		: css_value
																		)
																	+ ';'
												: '';
	...
	function stargaze_customizer_add_theme_vars(vars) {
		var rez = [];
		if ( typeof vars['rad'] != 'undefined' ) {
			if (vars['rad'] == '') {
				vars['rad'] = 0;
			}
			rez['rad']      = stargaze_customizer_prepare_css_value( vars['rad'] );
		...
		if ( typeof vars['page_width'] != 'undefined' ) {
			...
			if ( isNaN( vars['page_width'] ) || vars['page_width'] === 0) {
				vars['page_width'] = stargaze_customizer_vars['page_width_default'];
			}
			vars['sidebar_width']       = parseInt( vars['sidebar_width'], 10 );
			vars['sidebar_gap']         = parseInt( vars['sidebar_gap'], 10 );

			rez['page_width']          = stargaze_customizer_prepare_css_value( vars['page_width'] );
			rez['page_boxed_extra']     = stargaze_customizer_prepare_css_value( vars['page_boxed_extra'] );
			rez['page_fullwide_extra']  = stargaze_customizer_prepare_css_value( vars['page_fullwide_extra'] );
			rez['page_fullwide_max']    = stargaze_customizer_prepare_css_value( vars['page_fullwide_max'] );
			rez['grid_gap']             = stargaze_customizer_prepare_css_value( vars['grid_gap'] );
			rez['sidebar_prc']          = vars['sidebar_width'] / vars['page_width'];
			rez['sidebar_gap_prc']      = vars['sidebar_gap'] / vars['page_width'];
			rez['sidebar_width']        = stargaze_customizer_prepare_css_value( vars['sidebar_width'] );
			rez['sidebar_gap']          = stargaze_customizer_prepare_css_value( vars['sidebar_gap'] );
			rez['sidebar_gap_width']    = stargaze_customizer_prepare_css_value( vars['sidebar_gap'] );
	...
	// Add ed to css value
	function stargaze_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).slice( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function stargaze_customizer_prepare_html_value(val) {
